...
function Register(props) {
    ...
    const handleSubmit  = () => {
        ...
        if (displayName.length < 2) {
            document.getElementById("ValidateDisplayName").innerHTML = 'Display name too short(needs to be 2 or more letters)';
            isValid = false;
        }
        else {
        ...
        <form name='MyForm'>
            <div className="field-name d-grid gap-0.8 central-frame">
                ...
                <div className="mb-3">
                    <div className="row">
                        <div className="col-sm-3">
                            <label htmlFor="exampleFormControlInput1" className="form-label field-label">Display Name</label><br/>
                        </div>
                        <div className="col-sm-9">
                            <label htmlFor="exampleFormControlInput1" id="ValidateDisplayName" className="form-label validation-label"></label><br/>
                        </div>
                    </div>
                    <input type="text" className="form-control" id="displayname" value={displayName} placeholder="My Display Name"  onChange={e => setDisplayName(e.target.value)}></input>
                </div>
                <div className="mb-3">
                    <div className="row">
                        <div className="col-sm-3">
                ...
                <div className="row">
                        <div className="col-sm-3">
                            <label htmlFor="exampleFormControlInput1" className="form-label field-label">Confirm Password</label><br/>
                        </div>
                        <div className="col-sm-9">
                            <label htmlFor="exampleFormControlInput1" id="ValidateConfirmPassword" className="form-label validation-label"></label><br/>
                        </div>
                    </div>
                    <input type="password" className="form-control" id="conf" value={confirmPassword} rows="3" placeholder='Confirm Password' onChange={e => setConfirmPassword(e.target.value)}></input>
                </div>
                <div>
                    <button type="button" className="btn btn-link">Already registered? log in here</button>
                </div>
                <div className="d-grid gap-2">
                    <button className="btn btn-outline-success" type="button" onClick={()=>handleSubmit()}>Submit</button>
